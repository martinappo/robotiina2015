...
RefereeCom::RefereeCom(boost::asio::io_service &io_service, std::string port, unsigned int baud_rate, const std::string &name)
	: SimpleSerial(io_service, port, baud_rate), ThreadedClass(name), ConfigurableModule(name)
{
...
void RefereeCom::Run() {
	...
	while (!stop_thread){
		...
		std::this_thread::sleep_for(std::chrono::milliseconds(1000)); // do not poll serial to fast
	}
